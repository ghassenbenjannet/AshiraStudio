...
export const concurrentSchema = baseEntitySchema.extend({
  ...
  segment: z.string().max(100).nullable(),
...
export const expressionSchema = baseEntitySchema.extend({
  ...
  transliteration: z.string().max(300).nullable(),
  registre_id: z.string().uuid(),
  statut: z.enum(STATUT_EXPRESSION).default("a_valider"),
  contexte_usage: z.string().max(500).nullable(),
  exemple: z.string().max(500).nullable(),
  ajoutee_par: z.string().uuid().nullable(), // null si auto-proposée par un agent (RG-CU2)
  validee_par: z.string().uuid().nullable(),
});
