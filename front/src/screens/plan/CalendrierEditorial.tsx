...
import { tacheEnRetard, type Tache, type Contenu } from "@achirah/shared";
...
/** E18 — Calendrier éditorial (vue mois) : tâches/shootings + contenus planifiés/publiés (date_publication). */
export function CalendrierEditorial({
  ...
  onOuvrirContenu?: (id: string) => void;
}) {
  ...
  const evenementsParJour = useMemo(() => {
    const carte = new Map<string, EvenementCalendrier[]>();
    const ajouter = (date: string, evenement: EvenementCalendrier) => {
      const liste = carte.get(date) ?? [];
      liste.push(evenement);
      carte.set(date, liste);
    };
    taches.forEach((tache) => {
      ajouter(tache.date_echeance, {
        id: tache.id,
        titre: tache.titre,
        infobulle: `${tache.titre} — ${campagneParId.get(tache.campagne_id) ?? ""}`,
        couleur: COULEUR_TYPE[tache.type] ?? COULEUR_TYPE.autre!,
        enRetard: tacheEnRetard(tache, new Date().toISOString().slice(0, 10)),
        ouvrir: () => onOuvrirTache(tache.id),
      });
    });
    contenus.forEach((contenu) => {
      if (!contenu.date_publication) return;
      const date = contenu.date_publication.slice(0, 10);
      ajouter(date, {
        id: contenu.id,
        titre: contenu.titre,
        infobulle: `${contenu.titre} — ${campagneParId.get(contenu.campagne_id) ?? ""}`,
        couleur: COULEUR_TYPE.contenu!,
        enRetard: false,
        ouvrir: () => onOuvrirContenu?.(contenu.id),
      });
    });
    return carte;
          ...
          {semaines.map((semaine, i) => (
            <div key={i} className="grid grid-cols-7 gap-1">
              {semaine.map((jour) => {
                ...
                const evenements = evenementsParJour.get(iso) ?? [];
                return (
                  <div key={iso} className={`min-h-24 rounded-field border border-line p-1 ${dansLeMois ? "bg-panel" : "bg-transparent opacity-40"} ${iso === aujourdhui ? "border-sable" : ""}`}>
                    ...
                    {evenements.slice(0, 3).map((ev) => (
                      <button
                        ...
                        onClick={ev.ouvrir}
                        title={ev.infobulle}
                        className={`mb-0.5 block w-full truncate rounded-field px-1 py-0.5 text-start text-[11px] text-bg ${ev.couleur} ${ev.enRetard ? "ring-1 ring-danger-fg" : ""}`}
                      >
                        {ev.titre}
                      </button>
                    ))}
