...
export function Aujourdhui() {
  ...
  const chapitreActif = campagnes.find((campagne) => campagne.statut === "active");
      ...
      <section className="relative mb-5 overflow-hidden rounded-[20px] bg-sable p-5 text-white sm:p-6">
        ...
        <div className="relative z-10 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            {chapitreActif && (
              <Link to={`/plan/campagnes/${chapitreActif.id}`} className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-white/90 hover:text-white">
                <span>{chapitreActif.nom}</span>
                <span className="rounded-full bg-white/15 px-2 py-1 text-xs">J{joursRestants(chapitreActif.date_fin) >= 0 ? "-" : "+"}{Math.abs(joursRestants(chapitreActif.date_fin))}</span>
